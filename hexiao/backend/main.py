...
def process(delivery_path):
    pd = load_prices()
    rows = extract(delivery_path); pl, sh = pd.get('prices',[]), pd.get('shipping',{}); res = []
    for row in rows:
...
def import_prices():
    path = str(BASE.parent / '对账单5.4.xlsx')
    if not os.path.exists(path):
        path = str(BASE / '对账单5.4.xlsx')
    if not os.path.exists(path):
...
async def process_file(file: UploadFile = File(...)):
    # save uploaded file
    tmp = BASE / 'temp_upload.xlsx'
    content = await file.read()
    with open(tmp, 'wb') as f: f.write(content)
    result = process(str(tmp))
    os.remove(tmp)
    return result

# serve static files
@app.get('/')
def index(): return FileResponse(str(WEB_DIR / 'index.html'))
